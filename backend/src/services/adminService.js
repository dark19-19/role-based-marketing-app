...
class AdminService {
  ...
  async resetUserPassword({ adminUser, targetUserId, newPassword }) {
    try {
      ...
      const updated = await userRepo.updatePassword(targetUserId, hash);
      if (!updated) throw new Error("المستخدم المستهدف غير موجود");
  ...
  async createUser(
    currentUser,
    { first_name, last_name, phone, password, role, branch_id, supervisor_id },
  ) {
    first_name = isString(first_name, "يرجى إدخال اسم أول صحيح");
    last_name = isString(last_name, "يرجى إدخال اسم ثاني صحيح");
    phone = isString(phone, "رقم الهاتف مطلوب");
    ...
    if (creatorRole !== "ADMIN") {
      ...
      if (
        ...
        !allowedRoles[creatorRole].includes(targetRole)
      ) {
